...
module ActiveRecord
  class Migration
    class << self
      # Raises <tt>ActiveRecord::PendingMigrationError</tt> error if any
      # migrations are pending.
      def check_pending!(connection = Base.connection)
        return unless ActiveRecord::Migrator.needs_migration?(connection)
        raise StandardError,
              "Migrations are pending. To resolve this issue, run:\n\n\
          bin/rails db:migrate RAILS_ENV=#{::Rails.env}\n\n\
              Missing migrations: \
              #{ActiveRecord::Migrator.missing_migrations}\n"
      end
    end
  end
